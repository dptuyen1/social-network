...
const Post = ({ props }) => {
    ...
    useEffect(() => {
        ...
        const loadDetails = async () => {
            let res = await detailsService.load(props.id);
    ...
    const addReaction = (reactionId) => {
        const process = async () => {
            try {
                let r = await authApi().get(endpoints['existed-detail'](user.id, props.id));

                if (r.status === 200) {
                    try {
                        let res = await authApi().post(endpoints['add-reaction'], {
                            postId: props.id,
                    ...
                    try {
                        ...
                        setDetails((prevDetails) => {
                            ...
                            const detailIndex = updatedDetails.findIndex((detail) => detail.id === id);

                            if (detailIndex !== -1) {
                                updatedDetails[detailIndex].reactionId.id = res.data.reactionId.id;
                            }
    ...
    const deleteReaction = (reactionId) => {
        const process = async () => {
            try {
                await authApi().delete(endpoints['delete-detail'](reactionId));

                setDetails((prevDetails) => prevDetails.filter((detail) => detail.id !== reactionId));
            } catch (error) {
                console.log(error);
            }
        };
    ...
    return (
        <div className="d-flex justify-content-center align-items-center">
            <div className="shadow-sm px-4 py-3 my-3 bg-white rounded w-50">
                ...
                <div className="d-flex justify-content-between">
                    {details.length > 0 && <p>{details.length} lượt thích</p>}
                    ...
                    <div className="row gap-2 py-2">
                        ...
                        >
                            <div className="btn btn-light col d-flex align-items-center justify-content-center gap-3">
                                {details.filter((detail) => detail.userId.id === user.id).length === 0 ? (
                                    <div
                                    ...
                                    details
                                        .filter((detail) => detail.userId.id === user.id)
                                        .map((detail) => (
                                            <div
                                                key={detail.id}
                                                className="d-flex align-items-center justify-content-center gap-3 text-primary"
                                                onClick={() => deleteReaction(detail.id)}
                                            >
                                                <img
                                                    className="img-20"
                                                    src={detail.reactionId.icon}
                                                    alt="reaction-icon"
                                                ></img>
                                                <p className="mb-0">{detail.reactionId.name}</p>
                                            </div>
                                        ))
                                )}
                            </div>
