    ...
    return (
        <>
            <div className="card shadow-sm p-2 m-auto my-4" style={{ width: '30rem' }} key={post.id}>
                <div className="post-header d-flex gap-3 align-items-start">
                    <img className="rounded-circle" src={post.userId.avatar} alt="" style={imgStyle}></img>
                ...
                <Comment user={user} comment={comments} imgStyle={imgStyle} />

                <Input user={user} imgStyle={imgStyle} />
            </div>
        </>
    );
};
